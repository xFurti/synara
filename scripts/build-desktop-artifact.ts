...
const stampStagedDesktopFlavor = Effect.fn("stampStagedDesktopFlavor")(function* (
  ...
  if (source.includes(CANARY_DESKTOP_FLAVOR_FALLBACK)) {
    yield* Effect.log("[desktop-artifact] Staged Electron main bundle has Canary flavor.");
    return;
  }
  ...
  if (process.platform === "win32") {
    ...
    if (options.flavor === "canary") {
      ...
      if (yield* fs.exists(nodePtyDir)) {
        yield* fs.writeFileString(
          path.join(nodePtyDir, "Directory.Build.props"),
          disableSpectreProps,
        );
      }
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  flavor: Flag.choice("flavor", BuildFlavor.literals).pipe(
    Flag.withDescription(
      "Packaged identity: production (default) or canary for an isolated test app.",
    ),
    Flag.optional,
  ),
}).pipe(
